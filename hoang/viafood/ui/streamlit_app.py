...
from ui.voice_utils import record_voice_to_text, speak_text
...
class StreamlitUI:
    ...
    def render_chat_interface(self):
        ...
        if st.button("🎙️ Ghi âm câu hỏi"):
            user_input = record_voice_to_text()
            if user_input:
                st.session_state.user_input = user_input
                st.success(f"✅ Bạn nói: {user_input}")
                st.session_state.trigger_submit = True
        ...
        # Nút phát lại câu trả lời
        if st.session_state.chat_history:
            last_message = st.session_state.chat_history[-1]
            if last_message['role'] == 'bot':
                if st.button("🔊 Nghe câu trả lời"):
