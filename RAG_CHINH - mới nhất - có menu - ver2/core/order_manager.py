...
class Order:
    def __init__(self, user_id: str):
        ...
        self.status = "pending"  # pending, confirmed, sent_to_kitchen, completed, paid
        self.note = ""
...
class OrderManager:
    ...
    def get_current_order(self, user_id: str) -> Order:
        ...
        return self.get_pending_order(user_id) or self.create_order(user_id)

    def get_pending_order(self, user_id: str) -> Optional[Order]:
        """Lấy order có trạng thái 'pending' gần nhất, nếu không có thì trả về None."""
        user_orders = self.orders.get(user_id, [])
        for order in reversed(user_orders):  # Duyệt từ mới đến cũ
            if order.status == "pending":
                return order
        return None

    def create_order(self, user_id: str, table_id: str = None) -> Order:
    ...
    def add_dish(self, user_id: str, dish: VietnameseDish, quantity: int = 1, note: str = "", order_id: str = None):
        print(f"[DEBUG][add_dish] user_id={user_id} | dish={dish.name} | quantity={quantity} | order_id={order_id}")
        if order_id:
            # Nếu có order_id, tìm order cụ thể
            user_orders = self.orders.get(user_id, [])
            order = next((o for o in user_orders if o.order_id == order_id), None)
            if not order:
                print(f"[WARNING] Order {order_id} not found, creating new order")
                order = self.create_order(user_id)
        else:
            # Nếu không có order_id, lấy hoặc tạo order pending
            order = self.get_current_order(user_id)
        
        order.add_item(dish, quantity, note)

    def remove_dish(self, user_id: str, dish_name: str, order_id: str = None):
        if order_id:
            # Nếu có order_id, tìm order cụ thể
            user_orders = self.orders.get(user_id, [])
            order = next((o for o in user_orders if o.order_id == order_id), None)
            ...
            # Nếu không có order_id, lấy order pending
            order = self.get_current_order(user_id)
