...
def default_demo_dishes() -> List[VietnameseDish]:
    return [
        VietnameseDish(
            name="Bún chả",
            region="Miền Bắc",
            description="Thịt nướng ăn kèm bún, rau sống và nước chấm chua ngọt.",
            ingredients="Thịt ba chỉ, bún, rau thơm, nước mắm chua ngọt",
            recipe="",
            price=45000,
            unit="tô",
            cook_time=20,
        ...
        VietnameseDish(
            name="Bánh xèo",
            region="Miền Nam",
            description="Bánh vàng giòn, nhân tôm thịt giá, ăn kèm rau và nước chấm.",
            ingredients="Bột gạo, nghệ, tôm, thịt heo, giá, rau sống",
            recipe="",
            price=40000,
            unit="cái",
            cook_time=25,
...
def answer_with_llm_strict(query: str, top_k: int = 5):
    ...
    # 2) Tạo context ngắn gọn + danh sách allowed
    ...
    try:
        ...
        # Nếu LLM trả sai định dạng => xem như không hợp lệ
        valid_picks = []

    print("\n===== LLM ANSWER (STRICT) =====")
    if valid_picks:
        # In ra câu trả lời an toàn (chỉ món hợp lệ)
        for i, p in enumerate(valid_picks, 1):
            print(f"{i}. {p['name']} — {p['why']}")
    else:
        # Fallback: in thẳng top-k từ RAG, đảm bảo không bịa
...
def run_repl(mode: str):
    ...
    while True:
        ...
        # Mặc định: coi input là câu hỏi và chạy RAG+LLM theo mode
        if mode == "strict":
...
def main():
    ...
    if args.excel:
        dishes = try_load_excel(args.excel)
    if not dishes:
        dishes = default_demo_dishes()
