...
from intent.intent_history import check_history_intent, handle_history_query
from models.db_helpers import get_used_dishes_history
import re
import random
import unicodedata
import difflib
...
class VietnameseFoodChatbot:
    ...
    def chat(self, user_message: str, user_id: str = "default", order_id: str = None) -> str:
        ...
        # Kiểm tra intent lịch sử món ăn
        if check_history_intent(user_message):
            history_response = handle_history_query(user_message, user_id)
            self.conversation_history.append(ChatMessage(role="assistant", content=history_response))
            return clean_star_lines(history_response)
