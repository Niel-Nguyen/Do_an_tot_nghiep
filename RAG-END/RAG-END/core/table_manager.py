...
class TableManager:
    def __init__(self, db_manager=None):
        ...
        # Load tables from database if available
        if self.db_manager:
        ...
        try:
            ...
            for table_data in tables_data:
                table = Table(
                    ...
                    created_at=datetime.fromisoformat(table_data['created_at']) if table_data.get('created_at') else datetime.now(),
                    updated_at=datetime.fromisoformat(table_data['updated_at']) if table_data.get('updated_at') else datetime.now()
                )
    ...
    def create_table(self, name: str, capacity: int, location: str) -> Table:
        ...
        if self.db_manager:
            try:
                ...
                print(f"Saved table {table.name} to database")
            except Exception as e:
    ...
    def find_table_by_user_context(self, user_id: str) -> Optional[Table]:
        """Tìm bàn dựa trên context của user (IP, session, etc.)"""
        # Tìm theo session active của user này
        for session in self.sessions.values():
            if session.status == 'active':
                # Có thể user_id là IP và cần tìm session tương ứng
                # Hoặc có cách mapping khác
                table = self.get_table(session.table_id)
                if table:
                    return table
        
        # Nếu user_id là IP local, default về bàn 1
        if "192.168" in str(user_id) or "127.0.0.1" in str(user_id):
    ...
    def end_table_session(self, table_id: str) -> bool:
        ...
        print(f"Ending session for table {table_id}, token: {active_session.session_token[:8] if active_session.session_token else 'None'}...")
        
        active_session.end_time = datetime.now()
        active_session.status = "closed"
        active_session.session_token = None  # Invalidate token
        ...
        print(f"Session ended for table {table_id}")
        return True
        ...
        if table_id in self.tables:
            ...
            if self.db_manager:
                try:
                    ...
                    print(f"Deleted table {table_id} from database")
                except Exception as e:
                    print(f"Error deleting table from database: {e}")
    ...
    def get_table_by_token(self, token: str) -> Optional[dict]:
        """Lấy thông tin bàn theo token session"""
        for session in self.sessions.values():
            if session.session_token == token and session.status == "active":
                table = self.get_table(session.table_id)
                if table:
                    return {
                        'table_id': table.id,
...
# Sẽ được cập nhật từ app.py để truyền db_manager
table_manager = None
