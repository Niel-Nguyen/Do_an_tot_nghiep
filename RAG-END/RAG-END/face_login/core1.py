...
def enhance_image_quality(image):
    ...
    try:
        ...
        # 4. Ensure proper color space and bit depth
        ...
        if image.dtype != np.uint8:
            if image.dtype == np.float32 or image.dtype == np.float64:
                # If float, assume it's normalized to [0,1]
                image = (image * 255).astype(np.uint8)
                print(f"Converted from {image.dtype} to uint8")
            elif image.dtype == np.uint16:
                # If 16-bit, scale down to 8-bit
                image = (image / 256).astype(np.uint8)
                print("Converted from 16-bit to 8-bit")
            else:
...
def recognize_user(image_b64: str, table_id: str = None, session_token: str = None) -> str:
    ...
    try:
        ...
        print(f"Enhanced image processed, shape: {image.shape}, dtype: {image.dtype}")

        # Validate image format before face recognition
        if image is None:
            print("❌ Image is None after processing")
            return None
        
        if len(image.shape) != 3 or image.shape[2] != 3:
            print(f"❌ Invalid image shape: {image.shape}. Expected 3D RGB image")
            return None
            
        if image.dtype != np.uint8:
            print(f"❌ Invalid image dtype: {image.dtype}. Expected uint8")
            return None
