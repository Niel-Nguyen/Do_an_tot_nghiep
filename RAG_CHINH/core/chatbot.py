...
class VietnameseFoodChatbot:
    ...
    def chat(self, user_message: str, user_id: str = "default") -> str:
        ...
        # Nhận diện yêu cầu xem hóa đơn/bill (ưu tiên intent cứng)
        bill_phrases = ["xem hóa đơn","in bill", "xem bill", "hóa đơn", "bill", "tính tiền", "tổng tiền"]
        if any(phrase in user_message.lower() for phrase in bill_phrases):
            ...
            if not bill['items']:
                ...
                reply += "Nếu bạn muốn thanh toán, chỉ cần chọn 'Chuyển khoản' hay 'Tiền mặt' hoặc muốn thêm món gì nữa cứ nói nhé?" + confirm_order_button
        ...
        qr_phrases = [
            "chuyển khoản", "thanh toán chuyển khoản", "qr banking", "quét mã qr", "thanh toán qua ngân hàng"
        ]
        if any(phrase in user_message.lower() for phrase in qr_phrases):
            ...
            if not bill['items']:
                ...
                qr_img_url = f"https://img.vietqr.io/image/bidv-5811471677-qr_only.png?amount={total_amount}&addInfo=Thanh%20toan%20don%20{order_id}&accountName=NGUYEN%20NGOC%20PHUC"
                reply = (
                    f"💳 Vui lòng quét mã QR dưới đây để thanh toán chuyển khoản.<br>"
                    f"<img src='{qr_img_url}' alt='QR Banking' width='180'><br>"
                    f"Số tiền: <b>{total_amount:,}đ</b><br>"
                    f"Nội dung chuyển khoản: <b>Thanh toán đơn hàng {order_id}</b><br>"
                    f"Sau khi chuyển khoản, hãy nhấn 'Tôi đã thanh toán' để xác nhận nhé!"
                )
            self.conversation_history.append(
                ChatMessage(role="assistant", content=reply)
            )
        ...
        order_phrases = [
            "gọi", "order", "đặt", "cho tôi", "thêm", "thêm món", "lấy", "muốn ăn", "ăn", "mua", "add"
        ]
        # Chỉ xử lý intent order nếu user_message chứa từ khóa gọi món
        ...
        if any(phrase in user_message.lower() for phrase in confirm_order_phrases):
            order_manager.update_bill_status(user_id, "confirmed")
        ...
        try:
            ...
            if intent['type'] == 'contact':
                try:
                    ...
                    return clean_star_lines(insert_dish_images(reply))
            # Nếu user xác nhận, và đã có last_dish, thì thêm vào order trước khi trả lời chi tiết về món đó
            if is_confirm and self.last_dish:
                ...
                bot_response = self._process_llm_response(response, intent, context, user_message)
                self.conversation_history.append(
    ...
    def _extract_dish_name(self, user_message: str) -> str:
        # Chuẩn hóa: bỏ dấu, lower, loại ký tự đặc biệt
        ...
        for dish in rag_system.dishes_lookup.values():
            ...
            if dish_norm in user_norm:
                return dish.name
        # Nếu không match tuyệt đối, thử match fuzzy: tất cả từ khóa chính của tên món đều xuất hiện trong câu hỏi
        for dish in rag_system.dishes_lookup.values():
            dish_norm = normalize(dish.name)
...
CHỈ ĐƯỢC PHÉP TRẢ LỜI DỰA TRÊN DANH SÁCH MÓN ĂN DƯỚI ĐÂY (context). TUYỆT ĐỐI KHÔNG ĐƯỢC TỰ NGHĨ RA MÓN ĂN, GIÁ, NGUYÊN LIỆU, DINH DƯỠNG, CÁCH LÀM, V.V. NGOÀI DANH SÁCH NÀY. KHI GỢI Ý THỰC ĐƠN THEO GIÁ TRỊ (VD: ĐỦ 1 TRIỆU ĐỒNG), CHỈ ĐƯỢC PHÉP CHỌN MÓN TỪ DANH SÁCH CONTEXT, KHÔNG ĐƯỢC BỊA RA MÓN MỚI.\nNếu không có món phù hợp, hãy xin lỗi và gợi ý khách hỏi món khác.\n\nCONTEXT - Thông tin món ăn liên quan:\n{context}\n\nPHÂN TÍCH CÂU HỎI:\n- Loại câu hỏi: {intent['type']}\n- Từ khóa chính: {', '.join(intent['keywords'])}\n- Bộ lọc: {str(intent['filters']) if intent['filters'] else 'Không có'}\n"""
    ...
    def _process_llm_response(self, response: Any, intent: Dict[str, Any], context: str, user_message: str = "") -> str:
        try:
            ...
            return clean_star_lines(insert_dish_images(content))
        except Exception as e:
    ...
    def auto_confirm_pending_order(self, user_id):
        ...
        if pending_items:
            ...
            for item in pending_items:
                dish_name = normalize(item['dish'])
                matched_dish = None
                for key in rag_system.dishes_lookup.keys():
                    if normalize(key) == dish_name:
                        matched_dish = rag_system.dishes_lookup[key]
                        print(f'[DEBUG] Matched dish: {key} <-> {item["dish"]}')
                        break
                if matched_dish:
            ...
            self.last_suggested_dishes = []  # Clear sau khi chốt
            if added_dishes:
                reply = (
                ...
                reply = (
                    "Em không tìm thấy món nào hợp lệ trong thực đơn gợi ý để thêm vào hóa đơn. "
                    "Bạn muốn chọn lại không ạ?"
                )
        else:
            reply = (
